...
export const assertMongoTransactionSupport = async () => {
  if (!env.features.requireMongoTransactions) return

  const admin = mongoose.connection.db?.admin()
  if (!admin) {
    throw new Error('Cannot verify MongoDB transaction support before connecting')
  }

  const hello = await admin.command({ hello: 1 })
  const supportsTransactions = Boolean(hello.setName || hello.msg === 'isdbgrid')
  if (!supportsTransactions) {
    throw new Error('Money flows require MongoDB replica set or mongos transaction support')
  }
}
