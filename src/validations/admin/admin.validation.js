...
export const adminProductsQuerySchema = Joi.object({
  ...
  status: Joi.string().valid(...PRODUCT_STATUS_ENUM, 'banned', 'deleted'),
...
export const adminRejectOrderDeliveryReportSchema = Joi.object({
  adminNote: Joi.string().trim().min(1).max(1000).required(),
})
...
export const adminReportExportQuerySchema = Joi.object({
  type: Joi.string()
    .valid('orders', 'users', 'shops', 'withdrawals', 'platform_ledger', 'vip_subscriptions')
    .required(),
