...
describe('Shop API', () => {
  ...
  it('should invite staff and staff can update shop after accepting invitation', async () => {
    ...
    expect(inviteRes.body.message).toBe('Đã gửi lời mời nhân sự qua email');
    expect(inviteRes.body.data).toEqual(
    ...
    const acceptRes = await request(app)
      .post(`/api/v1/shops/invitations/${inviteRes.body.data.invitationId}/action`)
    ...
    expect(alreadyStaffInviteRes.statusCode).toBe(409);
  ...
  it('should reject staff invitation when email does not exist', async () => {
    const createRes = await request(app)
      .post('/api/v1/shops')
      .set('Authorization', `Bearer ${ownerToken}`)
      .send({ name: 'Missing Email Invite Shop' });
    ...
    const res = await request(app)
      .post(`/api/v1/shops/${shopId}/invitations`)
      .set('Authorization', `Bearer ${ownerToken}`)
      .send({ email: 'missing-staff@example.com' });

    expect(res.statusCode).toBe(404);
    expect(res.body.success).toBe(false);
  });

  it('should reject staff invitation from a user who is not the shop owner', async () => {
    const createRes = await request(app)
      .post('/api/v1/shops')
      .set('Authorization', `Bearer ${ownerToken}`)
      .send({ name: 'Owner Guard Invite Shop' });
  ...
  it('should reject invalid invitation email input', async () => {
    const createRes = await request(app)
      .post('/api/v1/shops')
      .set('Authorization', `Bearer ${ownerToken}`)
      .send({ name: 'Invalid Email Invite Shop' });
