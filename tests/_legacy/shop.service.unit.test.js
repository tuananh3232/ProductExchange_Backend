...
jest.unstable_mockModule('../../src/utils/mail.util.js', () => ({
  ...
  sendShopStaffInvitationEmail: mockSendStaffInvitationEmail,
}))
...
describe('shop.service shop_owner role logic', () => {
  ...
  describe('staff invitation rules', () => {
    ...
    it('allows inviting a regular member as staff', async () => {
      ...
      expect(mockSendStaffInvitationEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          invitationLink: expect.stringContaining('invitationId=invitation-id'),
        })
      )
    ...
    it('rejects inviting with an email that does not exist', async () => {
      mockShopRepo.findById.mockResolvedValue(makeShop())
      mockUserFindOne.mockResolvedValue(null)

      await expect(sendInvitation('shop-id', ownerContext, 'missing@example.com')).rejects.toMatchObject({
        statusCode: 404,
    ...
    it('rejects inviting when the requester is not the current shop owner', async () => {
      mockShopRepo.findById.mockResolvedValue(makeShop())

      await expect(sendInvitation('shop-id', { _id: 'other-id', roles: [ROLES.SHOP_OWNER] }, 'invitee-id@example.com')).rejects.toMatchObject({
        statusCode: 403,
    ...
    it('rejects creating a duplicate pending invitation for the same shop and invitee', async () => {
      mockShopRepo.findById.mockResolvedValue(makeShop())
      mockUserFindOne.mockResolvedValue(makeUser('invitee-id', [ROLES.MEMBER]))
      mockShopInvitationRepo.findOne.mockResolvedValue({
        _id: 'existing-invitation-id',
        status: INVITATION_STATUS.PENDING,
      })

      await expect(sendInvitation('shop-id', ownerContext, 'invitee-id@example.com')).rejects.toMatchObject({
        statusCode: 409,
        errorCode: 'A pending invitation already exists for this user and shop',
      })
      expect(mockShopInvitationRepo.create).not.toHaveBeenCalled()
      expect(mockSendStaffInvitationEmail).not.toHaveBeenCalled()
    })
